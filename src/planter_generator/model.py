...
def _outer_base_radius(z: float, config: GeneratorConfig) -> float:
    z_base = config.base_thickness_mm
...
def _texture_offset(z: float, theta: float, config: GeneratorConfig) -> float:
    ...
    if config.texture_type == TextureType.RINGS:
        return strength * sin(2.0 * pi * freq * z_ratio + phase)
    if config.texture_type == TextureType.SPIRAL:
        return strength * sin(freq * theta + 2.0 * pi * freq * z_ratio + phase)
    if config.texture_type == TextureType.GRID:
        return strength * sin(freq * theta + phase) * sin(2.0 * pi * freq * z_ratio + phase) * 0.5
...
def _build_planter_mesh(config: GeneratorConfig) -> trimesh.Trimesh:
    ...
    exact_base_z = z_base
    outer_base_index = z_levels.index(exact_base_z)
    inner_top_index = 0
    if outer_base_index < 0:
        outer_base_index = min(range(len(z_levels)), key=lambda i: abs(z_levels[i] - z_base))
    outer_base_start = outer_base_index * sections
    inner_base_start = inner_offset + 0 * sections
    for j in range(sections):
